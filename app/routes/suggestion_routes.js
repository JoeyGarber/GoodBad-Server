...
router.get('/suggestions', requireToken, (req, res, next) => {
  Suggestion.find({ owner: req.user.id })
    .then(suggestions => {
...
router.get('/suggestions/:id', requireToken, (req, res, next) => {
  ...
  Suggestion.findById(req.params.id)
    ...
    .then(suggestion => res.status(200).json({ suggestion: suggestion.toObject() }))
    // if an error occurs, pass it to the handler
    .catch(next)
})
...
router.patch('/suggestions/:id', requireToken, removeBlanks, (req, res, next) => {
  ...
  Suggestion.findById(req.params.id)
    ...
    .then(suggestion => {
      ...
      requireOwnership(req, suggestion)

      // pass the result of Mongoose's `.update` to the next `.then`
      suggestion.set(req.body.suggestion)
